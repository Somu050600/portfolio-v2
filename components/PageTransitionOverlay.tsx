...
/**
 * Handles all page transitions via the View Transitions API, the same pattern as
 ...
 *
 * ::view-transition-new(root) IS the new page, so the circle reveals real /home
...
export default function PageTransitionOverlay() {
  ...
  const handleCover = useCallback(
    async ({
      ...
      // Only suppress Next's scroll-to-top on a back-navigation, where our
      ...
      // viewports so the circle always covers, leaving no uncovered strip on mobile.
      ...
      // where they belong, so the reveal blooms up-left of the button instead of
  ...
  // Fire on initial mount for direct URL loads, where no VT animation runs, so
  ...
  useEffect(() => {
    ...
    // Hard nav / back-forward, with no VT animation in flight.
    if (!activeRef.current) {
